...
- **Step-8 herb-name mismatch (residual)**: term-level matching is in (see data-quality section above), but a herb still only scores when its `DrugSyndrome.HerbName` exactly equals the `Composition` herb name. Processed herbs don't match — the heading form `炙甘草` vs the composition form `甘草（炙）`. A normalization pass at match time (strip `（…）` from composition names + strip 炙/酒/炒/煅/醋 processing prefixes from both sides) would let those herbs count too. Behavior change — re-validate accuracy. (The deeper gap is patient↔drug vocabulary, e.g. patient `稀软` vs drug `大便稀` — that's the synonym-bridge item below.)
