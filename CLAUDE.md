...
- **Parsing bug discovered (deferred)**: 乌梅丸 and 理中汤 (and likely others) carry garbage `KeySymptoms` — the `方证匹配度评估` table (`匹配症状数|可靠性|建议`, a scoring guide, NOT symptoms) inside the 方证要点 section is parsed as KeySymptoms, inflating counts (乌梅丸 11 vs 8 real) and indexing noise terms (`可靠性`, `寒热错杂特点`). Doesn't affect the 厥阴 outcome (乌梅丸's real terms still don't match) but is a real data-quality bug — see Next Steps.
...
- **`方证匹配度评估` table parsed as KeySymptoms (data-quality bug)**: the matching-degree table inside the 方证要点 section (`匹配症状数|可靠性|建议`) is read as KeySymptoms — garbage rows like `匹配症状数`, `≥3条（含寒热错杂特点）`, `可靠性` inflate `len(KeySymptoms)` (used by the `candidateLess` specificity tiebreak) and get symptom-indexed. Discovered via the 厥阴 investigation (乌梅丸 + 理中汤 both affected). Fix is in the loader's 方证要点 extraction: stop at / skip a second table whose headers are the assessment schema, mirroring the repeated-header-row skip already used for 承气汤类 aggregate sub-tables. Validation-first: confirm `len(KeySymptoms)` drops to the real 方证对照表 row count and that accuracy stays 6/6 meridian / 5/5 family-aware (specificity tiebreaks may shift — re-run 5×).
