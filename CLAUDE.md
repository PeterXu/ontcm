...
- **Expand the accuracy benchmark beyond 5 cases (validation-first)**: the benchmark covers one formula each for 太阴/太阳/少阳/阳明/少阴 and **entirely skips 厥阴** (乌梅丸). It also lacks the differentiating counterpart within a meridian — 太阳表虚→桂枝汤 (validates the 麻黄/桂枝 step-7 tiebreak from the *other* side), 阳明经证→白虎汤 (vs 腑实→承气), 少阴热化→黄连阿胶汤 (vs 寒化→四逆). Adding these is read-only-first (write the case, observe pass/fail): a pass confirms robustness, a fail surfaces a real, benchmark-relevant, surgical fix. KB confirmed to carry 桂枝汤/白虎汤/黄连阿胶汤/乌梅丸. Highest-value single case: 厥阴/乌梅丸 (covers the untested 6th meridian).
