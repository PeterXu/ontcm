...
- **承气汤类 aggregate doc** (`yangming/chengqi_tang.md`): an overview comparing 大/小/调胃承气汤 with three sub-tables the parser merges into one. Loaded as a single "formula" with 16 aggregate KeySymptoms, it ties with `da_chengqi_tang` and makes the accuracy test's 阳明腑实 case nondeterministic. 调胃承气汤 (`tiaochengqi_tang`) exists ONLY as a sub-section here, so it can't just be deleted — needs splitting into a dedicated doc.
- **Accuracy test nondeterminism**: the 5 canonical cases' `方证 exact` is 4/5 or 5/5 run-to-run. The 承气汤 family ties on `MatchScore` and `sort.Slice` (non-stable) over map-ordered candidates picks nondeterministically. Fix with a stable sort + deterministic secondary key (e.g. prefer exact formula-name matches, or non-aggregate formulas).
