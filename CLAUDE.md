...
- **Step-8 matching is phrase-level, so activation is partial**: the drug-syndrome data now loads for both schemas (see data-quality section above) and step 8 fires, but its match `strings.Contains(symptom.Symptom, syndrome.TargetSymptom)` requires the patient symptom string to contain the drug's *entire* `TargetSymptom` phrase. Most TargetSymptoms are `、`-lists (`乏力、少气懒言`), while patient symptoms are `"<label>: <value>"` — so only bare single-term TargetSymptoms (e.g. 麻黄汤's `无汗`) match today. Splitting TargetSymptom on `、/，` and checking per-term would light up far more herbs. Related: herb-name mismatch (Composition `甘草（炙）` vs heading `炙甘草`) means processed herbs don't match — a normalization pass (strip processing `（…）` + 炙/酒/炒 prefixes) at match time would help. Both are behavior changes needing accuracy re-validation.
