...
- **Patient↔drug synonym bridge (step 8) — minimal, conservative table**: drug TargetSymptoms use formal/古 forms (`大便稀`, `食欲差`) while the 十问 wizard collects colloquial forms (`稀软`, `不想吃`) that share no characters, so term-level matching missed them despite denoting the same sign. Added `internal/agent/synonyms.go` — a directional table (formal drug term → colloquial patient aliases) consulted by `drugMatchesAnySymptom`. Only clinically-unambiguous, purely-additive mappings landed: `大便稀`↔{稀软,便溏,稀便,溏便,腹泻}, `食欲差`/`食少`/`纳呆`↔{不想吃,纳差,食欲不振,没胃口}. Deliberately OMITTED: a constipation bridge (`不大便`/`大便硬`↔`便秘`). It is clinically valid but operationally harmful — the 承气汤 docs record the sign inconsistently (大/调胃写 `不大便`, 小写 `大便硬`), so the alias would reward 大/调胃 over 小 by a data-completeness accident, breaking the 承气 tie that step 12 defers to the LLM (member depends on clinical SEVERITY, not on which synonym the doc used). 阳明 reaches the 承气 family via step-7 anyway. Vague overlaps (胸胁苦满 fullness vs 胁痛 pain) stay unmapped. Caught via `TestStep12_LLMRefinesTiedCandidates` failing when the constipation alias was first included — the failing test was the signal that the bridge manufactured a false winner, which is exactly the kind of interaction the accuracy re-validation gates for. Accuracy: 5/5 meridian, 5/5 family-aware, deterministic (5 runs). Regression tests: `TestDrugMatchesViaAlias`, `TestAliasesForDirectional`.


## Next Steps
All 8 phases are complete. Candidate follow-ups (data quality / enhancement):
- **Patient↔drug vocabulary bridge (residual)**: a conservative synonym table landed for step 8 (see data-quality section above — `大便稀`/`食欲差` families). The table is deliberately minimal; more mappings could close further gaps (e.g. `口干`↔`口渴`, sleep/energy terms). Each addition is a behavior change — re-validate accuracy and confirm it doesn't manufacture a false winner (the constipation-bridge case is the cautionary example: a valid synonym that broke the 承气 tie via doc inconsistency). Expanding the patient↔*formula* (step 7) vocabulary — currently only ClinicalSign + bigrams — is a separate, larger item.
