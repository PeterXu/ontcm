...
- Accuracy benchmark across 9 cases covering all 六经 + differentiating counterparts within a meridian (太阳表实/表虚, 阳明腑实/经证, 少阴寒化/热化): 定经 9/9, 方证 6/7 exact, 7/7 family-aware, deterministic (5 runs) (`diagnostic_accuracy_test.go`). 厥阴 + 少阴热化 added post-Phase-8 (see below); each is `meridianOnly` (its 主方 is not wizard-selectable — documented LLM/free-text gap).
...
## Differentiating counterparts — benchmark expansion (post-Phase-8)
Expanded the benchmark from 6 (one per 六经) to 9 cases, adding the differentiating counterpart *within* three meridians — the read-only-first validation pass recommended in Next Steps. Two passed outright; one surfaced a real gap (fixed below).
- **太阳表虚→桂枝汤 — PASS (exact)**: the counterpart of 太阳表实→麻黄汤. Both share 头痛/发热/脉浮, but 表虚 has 有汗+脉浮缓 vs 表实's 无汗+脉浮紧. 桂枝汤 wins over 麻黄汤 via the step-7 meridian+required bonus — and cleanly now that the `方证匹配度评估` fix (Loader data-quality section) stopped leaking 桂枝汤's "无汗" contrast-row as a KeySymptom (it no longer matches a 无汗 patient at all). Validates the 麻黄/桂枝 tiebreak from the *other* side.
- **阳明经证→白虎汤 — PASS (exact)**: the counterpart of 阳明腑实→承气汤. Both are 阳明热, but 经证 is 无形之热 (四大: 大热/大汗/大渴/脉洪大, NO 腑实) vs 腑实's 有形之热 (便秘+腹满痛+潮热). 白虎汤 wins on `大汗`+`高热` symptom match + meridian bonus, with no constipation to pull the 承气 family ahead. Validates the 经证/腑实 split.
- **少阴热化→黄连阿胶汤 — surfaced a real gap, FIXED for 定经 (see next section)**: the counterpart of 少阴寒化→四逆汤. Its 定经 was unreachable (heat signs bled to 阳明); its formula selection is a documented LLM-gap.

## 少阴热化 Meridian Reachability — FIXED (post-Phase-8)
- **少阴热化 was unreachable** (the 热化 sub-pattern of 少阴, vs the already-working 寒化→四逆汤 path). Surfaced by the differentiating-counterpart case 少阴热化→黄连阿胶汤: the case came back 阳明. 少阴 is the second 六经 meridian (after 厥阴) that single-sign counting cannot reach.
- **Root cause is structural, mirroring 厥阴**: the 十问 wizard maps EVERY 少阴 sign to a COLD presentation (嗜睡/口渴但不想喝/畏寒/尿频 — all 寒化). 少阴热化 is 阴虚火旺 (a HEAT pattern): its hallmarks (心烦不得卧→入睡难, 舌红, 脉细数) all bleed to 少阳/阳明 under the wizard's mappings, so the hint count can never give 少阴 a majority — exactly as 厥阴's cold/heat signs split across 太阴/阳明.
- **Fix — 阴虚-tongue detection in `executeStep6`**: when the tongue shows the 阴虚 pattern (舌红/绛, NOT the normal 淡红, + 无苔/剥苔 — pathognomonic for 阴虚, which in 六经 is 少阴热化) AND no 阳明实热 sign (黄苔 / 大汗 / 便秘·不大便 腑实) rules it out, override the count to 少阴 regardless of the 阳明-dominated hint count. 阳明实热 always carries 黄苔, never 少苔, so the stripped coating is the unambiguous differentiator. Placed BEFORE the 厥阴 rule (a 阴虚-tongue case is 少阴热化, not 寒热错杂); the two never collide on the canonical cases (厥阴's tongue is 薄白, not 无苔). Regression test: `TestStep6ShaoyinRehua` (6 cases incl. both guards — 实热 blocks the override; 黄苔 is not 阴虚; 寒化 still reaches 少阴 via plain count).
- **Benchmark now covers both 少阴 sub-patterns**: the 少阴热化/黄连阿胶汤 case (女, 50, 心烦失眠, 舌红无苔, 脉细数) brings 定经 to **9/9**, deterministic over 5 runs. The case is `meridianOnly` — see the formula gap below.
- **Formula gap (documented, NOT fixed — needs LLM/free-text intake, same class as 乌梅丸, but deeper)**: even with 定经 = 少阴, the engine surfaces NO formula candidate (step 7 returns empty → step 12 "no formula candidates"). `huanglian_ejiao_tang` is not wizard-selectable: its 方证 (`心烦不得卧`/`心烦失眠`) shares no whole-term OR rune-bigram with the wizard's `入睡难` — the only step-3 term a 热化 patient produces (舌红少苔/脉细数 are tongue/pulse, not step-3 evidence). This is deeper than 厥阴's gap (乌梅丸 at least lost to 理中汤 on cold-sign over-matching; 黄连阿胶汤 has NO competing match at all). Selecting the 少阴热化主方 needs the LLM / free-text intake path — notably `心烦` free text WOULD hit 黄连阿胶汤's `心烦` bigram (it indexes 心烦不得卧/心烦失眠). The benchmark tolerates the empty-candidate step-12 error for `meridianOnly` cases (`runFullDiagnostic` keeps the last good session so 定经 is still assertable) and logs the gap.
- Accuracy re-validated: **9/9 meridian, 6/7 exact, 7/7 family-aware, deterministic (5 runs)**.


## Next Steps
All 8 phases are complete. Candidate follow-ups (data quality / enhancement):
- **The bigger vocabulary lever is LLM/free-text intake, not more synonyms**: the step-8 synonym table is now substantially complete for safe, canonical-relevant mappings (see data-quality section above). The remaining patient↔drug and patient↔formula misses are NOT synonyms — they're symptoms that live in `PatientInfo.History`/`ChiefComplaint` free text (太阴 大便稀, 太阳 恶寒/怕冷, 少阴 下利/手脚凉) and never become `SymptomEvidence`, so no step-8/step-7 matcher sees them. Ingesting that text (rule-based chunking, or the LLM free-text intake item — Phase 4 future use) would unlock far more matching than any further synonym. Caveat (verified): for the 7 formula-eligible benchmark cases this is **benchmark-neutral** — the structured 十问 step-3 already carries those same signs (大便稀→`stool_shape=稀软`, etc.), and they pass 7/7 family-aware (6/7 exact); 少阴寒化 reaches 四逆汤 via 嗜睡+口渴但不想喝+淡白舌+沉微脉 even with `下利/手脚凉` history-only. BUT the LLM/free-text path is now the concrete lever for the TWO `meridianOnly` formula-selection gaps — 乌梅丸 (厥阴) and 黄连阿胶汤 (少阴热化), whose 主方 aren't wizard-selectable because their formal 方证 (`口渴多饮`/`饥饿但不想吃`; `心烦不得卧`/`心烦失眠`) share no whole-term OR rune-bigram with the wizard's colloquial terms (`口渴想喝水`/`不想吃`; `入睡难`). Notably, free text carrying `心烦` WOULD hit 黄连阿胶汤's `心烦` bigram (it indexes 心烦不得卧/心烦失眠). So the LLM path's value is no longer just real-world intake robustness — it's the only rule-safe route to the two documented formula gaps; history-ingestion overlaps it. Two deferred synonyms (`下利`↔`腹泻`, constipation) become safe to add once a canonical case exercises them.
- **Differentiating counterparts — DONE** (benchmark expanded 6→9 cases; see the "Differentiating counterparts" + "少阴热化 Meridian Reachability" sections above). 太阳表虚→桂枝汤 and 阳明经证→白虎汤 passed as exact matches (robustness confirmed); 少阴热化→黄连阿胶汤 surfaced and fixed the 少阴热化 定经 gap. The remaining gap is formula *selection* for the two `meridianOnly` cases (乌梅丸, 黄连阿胶汤) — both need the LLM / free-text intake path (the "bigger vocabulary lever" item directly above), not more rule-based matching.
